#!/bin/bash
# ===============
# Script Name: dv-session.sh
# Description: Interactive session manager with preview and management actions.
# Keybinding:  Prefix + s
# Config:      bind s run-shell -b "~/.config/tmux/scripts/dv/dv-session.sh"
...
while true; do
    ...
    if [[ "$key" == "ctrl-x" ]]; then
        if [[ "$target_session" == "NEW" ]]; then
            "$script_dir/dv-input.sh" --message "Cannot kill the 'New Session' item."
        else
            if "$script_dir/dv-input.sh" --confirm "Kill session '$target_session'?"; then
                current_session=$(tmux display-message -p "#{session_name}")
                if [[ "$target_session" == "$current_session" ]]; then
                    ...
                    if [[ -n "$other_session" ]]; then
                        ...
                        "$script_dir/dv-input.sh" --message "Session '$target_session' killed. Switched to '$other_session'."
                        continue
                    fi
                fi
                tmux kill-session -t "=${target_session}"
                "$script_dir/dv-input.sh" --message "Session '$target_session' killed"
            else
                "$script_dir/dv-input.sh" --message "Kill cancelled"
            fi
        fi
        continue
        ...
        if [ $? -eq 0 ] && [ -n "$sess_name" ]; then
            if tmux has-session -t "=${sess_name}" 2>/dev/null; then
                "$script_dir/dv-input.sh" --message "Session '$sess_name' already exists."
            else
        ...
        if [[ "$target_session" == "NEW" ]]; then
            "$script_dir/dv-input.sh" --message "Cannot rename the 'New Session' item."
        else
            unset new_name
            new_name=$("$script_dir/dv-input.sh" --title " Rename Session " "Enter New Name" "$target_session")
            if [ $? -eq 0 ] && [ -n "$new_name" ]; then
                if [[ "$new_name" == "$target_session" ]]; then
                    continue
                elif tmux has-session -t "=${new_name}" 2>/dev/null; then
                    "$script_dir/dv-input.sh" --message "Session '$new_name' already exists."
                else
