...
LOG_FILE="${LOG_DIR}/smart_update_${REMOTE_HOST}_${TIMESTAMP}.log"

exec > >(tee -i "$LOG_FILE") 2>&1

printMsg "=========================================================="
printMsg "START TIME:  $(date)"
printMsg "TARGET:      ${REMOTE_USER}@${REMOTE_HOST}"
if [ -n "$SSH_PORT" ]; then
    printMsg "PORT:        ${SSH_PORT}"
fi
printMsg "MODE:        ${EXEC_MODE^^}$( [ "$DRY_RUN" = true ] && echo " (Dry Run)" || echo "" )"
printMsg "LOG FILE:    ${LOG_FILE}"
printMsg "=========================================================="
...
echo "🔍 [REMOTE] Detected system layout: $OS"
...
echo "🌐 [REMOTE] Testing internet connectivity through proxy tunnel (port ${TUNNEL_PORT})..."

if ! test_connectivity; then
    echo "❌ [REMOTE] Network loop failed. Reverse proxy stream timed out." >&2
    exit 1
fi

echo "✅ [REMOTE] Tunnel validation successful!"

if [ "$DRY_RUN" = "true" ]; then
    echo "🛡️  [REMOTE] [DRY RUN] Connection verified. Skipping operations as requested."
    exit 0
fi

case "$EXEC_MODE" in
    shell)
        echo "🐚 [REMOTE] Launching interactive shell with proxy enabled..."
    ...
    command)
        ...
        ;;
    update)
        echo "📦 [REMOTE] Launching update cycle..."
        if [ "$OS" = "ubuntu" ]; then
            export DEBIAN_FRONTEND=noninteractive
            apt-get -o Acquire::http::Proxy="socks5h://127.0.0.1:${TUNNEL_PORT}" -o Acquire::https::Proxy="socks5h://127.0.0.1:${TUNNEL_PORT}" update
            
            UPGRADES=$(apt-get -s -o Acquire::http::Proxy="socks5h://127.0.0.1:${TUNNEL_PORT}" -o Acquire::https::Proxy="socks5h://127.0.0.1:${TUNNEL_PORT}" dist-upgrade 2>/dev/null | grep -E '^Inst' | wc -l)
            echo "📊 [REMOTE] Found $UPGRADES packages queued for installation."
            
            apt-get -o Acquire::http::Proxy="socks5h://127.0.0.1:${TUNNEL_PORT}" -o Acquire::https::Proxy="socks5h://127.0.0.1:${TUNNEL_PORT}" -o Dpkg::Options::="--force-confold" dist-upgrade -y
        elif [ "$OS" = "rhel" ]; then
            YUM_OUT=$(yum --setopt=proxy="socks5h://127.0.0.1:${TUNNEL_PORT}" check-update -q 2>/dev/null || true)
            UPGRADES=$(echo "$YUM_OUT" | grep -v '^$' | wc -l)
            echo "📊 [REMOTE] Found $UPGRADES packages queued for installation."
            
            yum --setopt=proxy="socks5h://127.0.0.1:${TUNNEL_PORT}" update -y
        else
            echo "❌ [REMOTE] Operating system '$OS' not supported for automated updates." >&2
            exit 1
        fi
        echo "🏁 [REMOTE] Package upgrades completed successfully."
        ;;
    *)
        echo "❌ [REMOTE] Unknown execution mode: $EXEC_MODE" >&2
        exit 1
        ;;
esac
EOF
...
# 4. Execute Remote Command via SSH PTY
REMOTE_BOOTSTRAP="TMP_SCRIPT=\$(mktemp /tmp/dv_remote_XXXXXX.sh); echo '$REMOTE_PAYLOAD_B64' | base64 -d > \"\$TMP_SCRIPT\"; chmod +x \"\$TMP_SCRIPT\"; sudo DRY_RUN='$DRY_RUN' TUNNEL_PORT='$TUNNEL_PORT' EXEC_MODE='$EXEC_MODE' COMMAND_PAYLOAD=$(printf '%q' "$COMMAND_PAYLOAD") \"\$TMP_SCRIPT\"; EXIT_CODE=\$?; rm -f \"\$TMP_SCRIPT\"; exit \$EXIT_CODE"
